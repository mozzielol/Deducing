...
class BNN_Conv(snt.AbstractModule):
    """
    ...
    def __init__(self, X, n_outputs, num_task=2, init_mu=0.0, init_rho=0.0, activation=tf.nn.relu,
                         last_activation=tf.nn.softmax,name="BNN_Conv"):
        super(BNN_Conv, self).__init__(name=name)
        ...
        self.layers = []
        self.x_placeholder = tf.placeholder(tf.float32, shape=X.shape)
        ...
        self.layers.append(BNNConvLayer(X,16,[6,6],2))
        X,_,_ = self.layers[0](X,tf.nn.relu)
        self.layers.append(BNNConvLayer(X,16,[6,6],2))
        X,_,_ = self.layers[1](X,tf.nn.relu)
        #self.layers.append(BNNConvLayer(X,32,[6,6],2))
        #X,_,_ = self.layers[2](X,tf.nn.relu)

        final_output_shape = X.shape 
        X = tf.reshape(X, shape=[-1, final_output_shape[1:].num_elements()])
        n_inputs = X.shape[1]    
        self.layers.append( BNNLayer(n_inputs, n_outputs, init_mu=init_mu, init_rho=init_rho,num_task=num_task))
    ...
    def set_fisher_graph(self,x,y_):
        ...
        for i in range(len(self.layers)):
            ...
            if i == self.num_conv_layers:
                final_output_shape = inputs.shape 
                inputs = tf.reshape(inputs, shape=[-1, final_output_shape[1:].num_elements()])
        self.y = inputs
